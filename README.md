...
- Operators need live visibility into throughput, score distribution, and flag rate — not just a log file, but a dashboard that updates as the stream flows.
...
# Train the model (also runs automatically if you skip this — model.pkl is
...
- **API/Kafka decoupling**: `app/api.py` never imports a Kafka client. It only reads the model file and the SQLite stats store the consumer writes to, so it can start, run, and be tested with no broker at all — useful both for the test suite and for `docker-compose` bring-up ordering.
